...
_NEUROMULE_SECURITY_POLICY = (
    ...
    "пользователя (например, 'Кто такой Пушкин', 'Что такое фотосинтез') "
    "НЕ являются попыткой взлома — отвечай по сути.\n"
...
_STANDARD_FREE_CORE = (
    ...
    "«почему?»), ты ОБЯЗАН взять контекст из истории (свой предыдущий ответ) и выполнить "
    "команду над ним (переписать, дополнить, объяснить). Запрещено просить пользователя "
    "прислать текст заново, если он есть в истории!\n"
    "3. Если пользователь явно продолжает прошлую тему, используй историю как справочный фон. "
    ...
    "4. Твой итоговый ответ должен оставаться лаконичным: до 2–3 емких абзацев, "
    ...
    "Сразу под ним выведи ровно 3 строки с контекстными вопросами-подсказками "
...
_CHATCOM_LACO_TAIL = (
    ...
    "Обязательно заверши ===КНОПКИ=== + ровно 3 follow-up (1–3 слова) по теме. "
...
_FREE_ROUTE_LOCK = (
    ...
    "ровно 3 коротких вопроса (1–3 слова по теме). Без COPY PACK и без <pre>-пачек."
)
...
_PAID_SUGGESTED_REPLIES_TAIL = (
    ...
    "и ровно 3 коротких follow-up (1–5 слов) по якорям из ответа — "
    "не общие «подробнее/пример/дальше». Стиль и длину ответа не меняй — "
    "оставайся в премиальном формате.]"
)
