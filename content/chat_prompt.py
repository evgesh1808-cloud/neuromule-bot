...
_STANDARD_REPLY_EXAMPLE_RULE = (
    "Примеры: любые готовые реплики или шаблоны фраз ОБЯЗАТЕЛЬНО упаковывай в тег:\n"
    "<blockquote expandable>📋 <b>Пример реплики:</b>\n"
    "<code>[текст]</code></blockquote>\n"
    "В ответе должно быть не меньше одной такой карточки реплики, когда даёшь готовые фразы "
    "для родителя/тренера/пользователя. Не оставляй реплики только «в кавычках» в абзаце — "
    "всегда дублируй ключевую фразу в блоке «Пример реплики».\n"
)
...
_PAID_STANDARD_COMPLIANCE_TAIL = (
    ...
    "В первом абзаце — живой человечный отклик; при уместности — «Маршрут» только как план/траектория "
    "(не «взвешивание маршрутов/нейронов»). "
    "Далее 3–5 блоков <b>1. Название</b> с эмодзи-подпунктами. "
    "Практичные сценарии и готовые реплики — тезисно, без дублей и воды. "
...
_PAID_STANDARD_ROLE_ADDON = (
    ...
    "1. Первый абзац — тёплый человечный отклик на контекст пользователя; при уместности "
    "«Маршрут» только как план действий / траектория развития / программа занятий "
    "(что делать дальше), никогда не «взвешивание маршрутов/нейронов».\n"
    "2. Затем 3–5 смысловых блоков: <b>1. Название блока</b>, внутри — подпункты "
    "с тематическими эмодзи в начале строки (😊, 🚀, ⚽️, 🗣️, 👨‍👩‍👧, ⏳ и т.п.).\n"
    "3. Обязательно: практические сценарии, упражнения, готовые фразы — тезисно, "
    "без дублирования мыслей внутри подпунктов.\n"
