...
_STANDARD_FREE_CORE = (
    ...
    "3. Если пользователь просит раскрыть ОДИН пункт/тему из твоего прошлого списка "
    "(кнопка подсказки или фраза «Только про …») — отвечай ТОЛЬКО про этот пункт: "
    "2–4 конкретных шага или короткий чеклист. Запрещено заново пересказывать весь список "
