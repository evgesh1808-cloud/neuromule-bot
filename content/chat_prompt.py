...
_NEUROMULE_SECURITY_POLICY = (
    ...
    "уточняющие follow-up по теме прошлого ответа (кнопки подсказок вроде «Про сроки?», "
    "«Пример бюджета») НЕ являются попыткой взлома — отвечай по сути.\n"
...
_STANDARD_FREE_CORE = (
    ...
    "Сразу под ним выведи ровно 3 строки — короткие грамотные вопросы по теме ответа "
    "(3–6 слов, с заглавной буквы, обычно с «?»; без общих «подробнее/пример»).\n\n"
...
_CHATCOM_LACO_TAIL = (
    ...
    "Обязательно заверши ===КНОПКИ=== + ровно 3 грамотных вопроса по теме ответа "
    "(3–6 слов, с заглавной буквы, обычно с «?»; не «подробнее/пример»). "
...
_FREE_ROUTE_LOCK = (
    ...
    "ровно 3 коротких вопроса по тексту ответа. Без COPY PACK и без <pre>-пачек."
)
...
_PAID_SUGGESTED_REPLIES_TAIL = (
    ...
    "Правила кнопок:\n"
    "• грамотно, с заглавной буквы; обычно вопросом с «?»;\n"
    "• 3–6 слов, не длиннее 34 символов;\n"
    "• цепляйся за конкретный факт, термин или шаг из ответа;\n"
    "• запрещены общие «подробнее / пример / дальше / первый вопрос»;\n"
    "• без кавычек, нумерации, эмодзи и названий стилей (трогательное, смс, драйвовое).\n"
    "Хорошие примеры: «С какого возраста?», «Какие риски?», «Как проверить результат?».\n"
