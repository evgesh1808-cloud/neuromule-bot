...
TXT_CREATE_IMAGE_AFTER_MODEL = (
    ...
    "Для <b>лицо + принт на одежду</b>: отправь <b>альбом из 2 фото</b> с подписью "
    "(1-е — вы взрослая, 2-е — принт или детское фото; например: «надень принт на худи» "
    "или «перенеси маленькую меня на футболку как принт»).\n"
    "Для <b>отражения в зеркале</b>: альбом из 2 фото + «покажи второе фото как отражение в зеркале».\n"
    "Для <b>нескольких людей на одном кадре</b> — кнопка «👨‍👩‍👧‍👦 Групповое фото».\n\n"
...
TXT_PHOTO_ALBUM_WAIT_CAPTION = (
    "📎 Альбом из 2 фото получен. Теперь одним сообщением опишите, что сделать.\n\n"
    "Примеры:\n"
    "• «перенеси маленькую меня с 2-го фото на футболку как принт»\n"
    "• «покажи 2-е фото как отражение в зеркале»"
)
...
TXT_PHOTO_ALBUM_GROUP_HINT = (
    "📎 Альбом из нескольких портретов получен. Добавьте <b>подпись</b> с расстановкой "
    "(например: «все вместе на фоне заката») или используйте кнопку "
    f"«{BTN_GROUP_PHOTO}» для пошагового сборщика."
)
