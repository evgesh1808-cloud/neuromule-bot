...
CB_HINT_BTN_PREFIX = "btn:"  # stateful: btn:<index>:<action_uuid>
...
BTN_DOWNLOAD_UNCOMPRESSED = "📥 Скачать без сжатия"
TXT_DOWNLOAD_UNCOMPRESSED_OK = "Файл подготовлен!"
TXT_DOWNLOAD_UNCOMPRESSED_GONE = "Файл устарел — сгенерируй фото ещё раз."
TXT_DOWNLOAD_UNCOMPRESSED_FAIL = "Не удалось отправить файл. Попробуй позже."
...
TEXT_ROLES: tuple[tuple[str, str], ...] = (
    ("⚪ Стандарт", "standard"),
    ("📄 Саммари", "summary"),
    (BTN_TEXT_ROLE_TABLE, "table_generator"),
...
#   ряд 1: ИИ-Ассистент | Изображение
...
TXT_HD_COMPAT_LOCKED = "❌ Сначала необходимо активировать личный Полный Разбор!"
TXT_MATCH_LOCKED = (
...
TXT_HD_UPGRADING_REPORT = (
    ...
    "Это бесплатно — ты уже оплатил полный разбор.\n"
    "Сверху должен гореть статус «печатает…» — генерация может занять до 3–5 минут. Не закрывай чат 🐎"
)
TXT_HD_UPGRADING_REPORT_ALERT = "Обновляем отчёт до новой версии…"
TXT_HD_UPGRADED_REPORT = (
    "<b>Разбор обновлён!</b> 🧬\n"
...
TXT_HD_REGENERATE_NEED_BIRTH = (
    ...
    "повторно 💎 не спишем.\n"
    "Пример: <code>12.09.1999, 5:20, Москва</code>"
)
...
TXT_HD_BTN_WEBAPP = "🗺️ Открыть Панель Управления (Super App)"
...
TARIFF_PLANS = (
    ...
    "• ⚡️ 10 Энергии/день — только для текстового чата (Стандарт: 1 ⚡/сообщение)\n"
    "• 🤖 Роль «Стандарт» в ИИ-Ассистенте\n"
    "• 🖼 GPT Image 2 — 1 качественное фото в день бесплатно, далее — 5 💎\n"
...
TXT_CHAT_RATE_LIMIT = (
    "⚠️ Не спамьте! Пожалуйста, подождите несколько секунд."
)
TXT_CHAT_BUSY = (
    "⏳ Я еще отвечаю на твой прошлый вопрос, подожди секунду!"
)
...
TXT_TABLE_GENERATOR_STATUS = (
    "⏳ Запускаю модуль <b>ИИ-Аналитик (Excel)</b>… "
    "Генерация и сборка отчёта займёт от 1 до 3 минут. Пожалуйста, подождите."
)
...
TXT_WB_FINANCE_INVALID_STRUCTURE = (
    "❌ <b>Ошибка распознавания файла</b>\n\n"
    "Структура колонок этого документа не соответствует оригинальному еженедельному "
    "финансовому отчёту (детализации) Wildberries.\n\n"
    "Пожалуйста, скачайте оригинальный файл из личного кабинета WB "
    "(Раздел: Финансовые отчеты -> Детализация) и отправьте его заново."
)
TXT_CHAT_DAILY_LIMIT = (
    "🔒 Ваша бесплатная энергия на сегодня закончилась! "
    "Обновление в полночь, либо активируйте MINI/SMART."
)
TXT_FREE_ENERGY_EXHAUSTED = TXT_CHAT_DAILY_LIMIT
TXT_RESET_OK = (
...
TXT_START_FIRST_MEET_NEED_CHANNEL_2 = (
    "• 🤖 ИИ-Ассистент: 1 ⚡️ за сообщение (10 ⚡️ обновляются ежедневно)\n"
    "• 🖼 GPT Image 2 — <code>{photo_daily_limit}</code> качественное фото в день бесплатно, далее — 5 💎\n\n"
...
TXT_START_WELCOME = (
    ...
    "<code>GPT-4o</code>, <code>Flux</code> и <code>Suno</code> в одном месте.\n\n"
    "Твой текущий статус: <b>FREE</b> 🎁\n"
    "• ⚡️ 10 Энергии — только для сообщений в текстовом ИИ-чате (сброс в 00:00)\n"
    "• 🖼 GPT Image 2 — 1 качественное фото в день бесплатно, далее — 5 💎\n\n"
...
TXT_START_PAYWALL = (
    ...
    "🎁 Ежедневно: 10 ⚡ Энергии на чат и 1 качественное фото GPT Image 2 в день бесплатно, далее — 5 💎!\n\n"
...
TXT_CHAT_ROLE_FALLBACK_STANDARD = (
    ...
    "🚀 Пополни 💎 или открой пакет MINI — и снова включай Саммари, "
    f"{TXT_AI_ANALYST_ROLE_PHRASE} и другие роли "
...
TXT_STD_REPLY_ZERO_BALANCE = (
    "⚠️ <b>У вас закончились суточные лимиты энергии на чат (0/10).</b>\n\n"
...
TXT_CREATE_IMAGE_AFTER_MODEL = (
    ...
    "Для <b>редактирования по одному фото</b>: отправь снимок (можно с подписью) "
    "или сначала фото, затем текстовый промпт.\n\n"
    "Для <b>нескольких людей на одном кадре</b>: отправь <b>альбом из 2–10 фото</b> "
    "с подписью-сценой или пришли фото по одному, затем опиши сцену текстом.\n\n"
    "Для <b>принта на одежде</b> (2 фото): альбом + подпись с словами "
    "«принт», «футболка», «на одежду» или «мерч».\n\n"
    "После результата — кнопка <b>✏️ Доработать</b> или ответ на картинку текстом."
)
TXT_PHOTO_REFINE_PROMPT = (
    "✏️ <b>Режим доработки активирован!</b>\n\n"
    "Вы можете:\n"
    "1. <b>Изменить текстом</b> — напишите, что поменять "
    "(например: «сделай куртку кожаной», «измени фон на пляж»).\n"
    "2. <b>Добавить принт/логотип</b> — отправьте <b>ОДНО</b> фото объекта "
    "и в подписи к нему напишите команду "
    "(например: «надень этот принт на футболку»).\n\n"
    "⚠️ <b>Важно:</b> отправляйте только <b>одно</b> новое фото за раз (не альбомом). "
    "Ваше исходное лицо со старого кадра я уже зафиксировал!"
)
TXT_PHOTO_REFINE_EXPIRED = (
    "Не найден последний результат для доработки. Сгенерируй новое фото или отправь снимок заново."
)
TXT_PHOTO_REFINE_ALBUM_REJECT = (
    "❌ Ой, вы отправили альбом из нескольких фото!\n\n"
    "В режиме доработки я могу принять только <b>одно</b> новое изображение предмета/принта, "
    "так как в качестве базы я автоматически использую ваш прошлый результат. "
    "Пожалуйста, пришлите только одно фото объекта <b>с подписью</b>."
)
TXT_PHOTO_ALBUM_WAIT_CAPTION = (
    "📎 Получено <b>2+ фото</b>. Опишите сцену одним сообщением — "
    "промпт уйдёт в генерацию как есть (например: «мама и дочка вместе на пляже»)."
)
TXT_PHOTO_MULTI_REF_COLLECT = (
    "📎 Фото сохранено. Можно отправить ещё портреты (до 10) или сразу описать сцену текстом."
)
TXT_PHOTO_ALBUM_TOO_MANY = (
    "❌ Для композита «лицо + принт» нужен альбом ровно из <b>2 фото</b> с подписью. "
    "Попробуйте снова или отправьте фото по одному."
)
TXT_PHOTO_ALBUM_GROUP_HINT = (
    "📎 Альбом из нескольких портретов получен. Добавьте <b>подпись</b> с описанием сцены "
    "(например: «мама и дочка на закате») или отправьте текст следующим сообщением."
)
TXT_PHOTO_REFINE_OBJECT_WAIT = (
    "📎 Фото для вставки получено. Теперь одним сообщением опиши, куда его поместить "
...
TXT_PHOTO_COMPOSITE_API_FAILED = (
    ...
    "<b>2-е — детское/принт</b>. Работает с <b>любой выбранной моделью</b> "
    "(Flux, GPT Image 2, Nano Banana).\n"
    ...
    "«✏️ Доработать» — только после <b>успешной</b> генерации одного фото."
)
...
TXT_GROUP_PHOTO_API_FAILED = (
    "⚠️ Не удалось собрать групповой кадр из {refs_count} фото — сервер OpenRouter "
    "не принял запрос или временно перегружен. "
    "Проверьте, что в промпте описана сцена (кто где стоит), и попробуйте ещё раз."
)
...
TXT_HD_ASK_BIRTH_DATA = (
    ...
    "• <b>Дата рождения</b>, <b>точное время</b> и <b>город</b> (для часового пояса).\n\n"
    "Пример: <code>12.09.1999, 5:20, Москва</code>\n"
    "Тип личности и бодиграф мы рассчитаем на сервере — указывать его не нужно."
)
TXT_HD_EMPTY_DATA = (
    "Не вижу данных. Пришли <b>дату</b>, <b>точное время</b> и <b>город</b> рождения одним сообщением.\n"
    "Пример: <code>12.09.1999, 5:20, Москва</code>"
)
...
TXT_HD_INSTAGRAM_ALBUM_CAPTION = (
    "🎁 {name}, твой персональный Инстаграм-пакет для сторис готов!\n\n"
    "Мы перевели коды твоих каналов на человеческий язык суперсил и упаковали их "
    "в неоновые карточки. Сохрани на телефон, делись в Stories и отмечай @neuromule_bot 🧬"
)
...
TXT_MATCH_PROCESSING = "💞 Расчет композита отношений..."
...
TXT_MEMORY_INTRO_EMPTY = (
    "🧠 <b>Память Нейронов</b>\n\n"
    "Пока у меня нет о вас сохраненных заметок.\n\n"
    "В процессе работы в режиме «Стандарт» я буду автоматически фиксировать ключевые факты о ваших проектах, стиле и задачах, чтобы точнее прокладывать Маршруты.\n\n"
    "<i>Вы можете записать воспоминание вручную, нажав кнопку ниже.</i>"
)
TXT_MEMORY_INTRO_FILLED = (
    "🧠 <b>Память Нейронов</b>\n\n"
    "Я зафиксировал и удерживаю в Системах следующие долгосрочные факты:\n\n"
    "<blockquote>{memory}</blockquote>\n\n"
    "<i>Вы можете отправить новый текст, чтобы перезаписать эти данные, или полностью очистить память.</i>"
)
TXT_MEMORY_PROMPT = (
...
TXT_MEMORY_BTN_SET = "📝 Добавить факт вручную"
...
TXT_GEN_STATUS_ACCEPTED = (
    "Мул ушёл в облака за ответом... 🐎☁️\n"
    "⏳ Доставка Flux FREE: 1–3 минуты. Пожалуйста, подождите."
)
...
TXT_CHAT_AI_UNAVAILABLE = (
    ...
    "<code>FREE_TEXT_MODEL=deepseek/deepseek-r1-distill-llama-8b:free</code>\n"
    "<code>FREE_MODELS=meta-llama/llama-3.1-8b-instruct:free,"
    "google/gemma-2-9b-it:free,qwen/qwen-2.5-7b-instruct:free</code>"
)
TXT_CHAT_AI_MAINTENANCE = (
    "⚙️ <b>Бесплатные шлюзы временно перегружены.</b> "
    "Попробуй через 15 минут или активируй MINI/SMART без очередей! 👑"
)
TXT_BLOGGER_AI_FAILED = (
    "😵 <b>Конструктор «Блогер» временно недоступен</b>\n\n"
    "Нейросеть не вернула пост (модель перегружена или устарели ID в <code>.env</code>). "
    "⚡ и 💎 за этот запрос <b>не списаны</b>.\n\n"
    "Попробуйте снова через минуту или упростите тему поста."
)

TXT_FREE_IMAGE_GLOBAL_CAP = (
    "Увы, бесплатный лимит фото на сегодня уже кто-то забрал ⏳ "
    "Но ты можешь запустить генерацию за Кристаллы прямо сейчас!"
)
TXT_FREE_IMAGE_CASCADE_FAILED = (
    "🤖 Все бесплатные нейросети сейчас перегружены запросами. "
    "Мы заботимся о вас, поэтому ваша ежедневная бесплатная попытка "
    "<b>СОХРАНЕНА</b> и не списалась. "
    "Пожалуйста, подождите 1–2 минуты и попробуйте сгенерировать снова!"
)
...
TXT_BLOGGER_COVER_READY = (
    ...
    '<blockquote expandable><b><a href="https://t.me/{bot_username}">NeuroMule</a></b>\n'
    "<code>{prompt}</code></blockquote>"
)
TXT_BLOGGER_COVER_FAILED = (
    "⚠️ Не удалось сгенерировать обложку. На стороне сервера произошел "
    "временный технический сбой, баланс полностью возвращён на ваш счёт. "
...
TXT_BLOGGER_COVER_INSUFFICIENT = (
    ...
    "На FREE — 1 фото GPT Image 2 в день бесплатно (меню «🎨 Фото»), далее — 5 💎."
)
...
TXT_ANIMATE_REPLICATE_FAILED = (
    ...
    "Кристаллы автоматически возвращены на баланс."
)
TXT_ANIMATE_OPENROUTER_QUOTA_FAILED = (
    "⚠️ <b>OpenRouter временно не может оживить фото</b>\n\n"
    "На аккаунте закончились кредиты для видео-генерации. "
    "<b>Кристаллы автоматически возвращены на твой баланс</b>. "
    "Попробуй позже или пополни баланс OpenRouter."
)
TXT_ANIMATE_OPENROUTER_FAILED = (
    "⚠️ Не удалось оживить фотографию через OpenRouter. "
    "Попробуй другой снимок или повтори через минуту.\n\n"
    "Кристаллы автоматически возвращены на баланс."
)
...
TXT_FAQ_ANSWER_ENERGY = (
    ...
    "• <b>На что она тратится:</b> каждое твое сообщение в «🤖 ИИ-Ассистенте» "
    ...
    "10 ⚡️ Энергии (обнуление в 00:00 по МСК). Если энергия закончилась, ты можешь купить "
...
TXT_FAQ_ANSWER_HD_SOURCE = (
    "🧬 <b>Как Мул рассчитывает Human Design? Это точно?</b>\n"
    "NeuroMule считает не «натальную карту», а <b>бодиграф Human Design (IHDS)</b>: "
    "тип, профиль, ворота, каналы и центры по времени рождения.\n"
    "• <b>Откуда берутся данные:</b> по дате, времени и городу рождения сервер "
    "считает положение тел (эфемериды Swiss Ephemeris) и собирает карту по правилам HD.\n"
    "• <b>Кто делает расшифровку:</b> платный полный разбор собирает нейросеть + "
    "статическую базу IHDS; «Совет дня» — короткий прогноз без тяжёлого LLM."
)
...
TXT_INSTRUCTION = (
    ...
    "⚡️ Энергия — для «🤖 ИИ-Ассистента» (сброс ежедневно). 💎 Кристаллы не сгорают.\n\n"
...
TXT_TARIFFS_MAIN = (
    ...
    "• ⚡️ <code>10 ⚡</code> Энергии на день <i>(строго для текстового чата "
    "«Стандарт»)</i>\n"
    "• 🖼️ 1 качественное фото <b>GPT Image 2</b> в день бесплатно, далее — 5 💎\n"
    "• 🔒 <i>Эксперт-чат, Саммари, 📊 ИИ-Аналитик (Excel), Подкасты, Музыка и Видео — полностью "
...
def format_admin_hd_report_notice_html(
    payer_uid: int,
    user_name: str,
    hd_type: str,
    birth_data: str,
) -> str:
    """Копия админу: готов полный разбор Human Design (не натальная карта)."""
    name_safe = html_module.escape((user_name or "клиент").strip() or "клиент")
    type_safe = html_module.escape((hd_type or "—").strip() or "—")
    birth_safe = html_module.escape((birth_data or "—").strip() or "—")
    return (
        "🧬 <b>Новый полный разбор Human Design</b>\n\n"
        f"👤 Клиент: {name_safe} (<code>{payer_uid}</code>)\n"
        f"🔮 Тип: {type_safe}\n"
        f"📅 Данные рождения: {birth_safe}\n\n"
        "Ниже — PDF-журнал и бодиграф (Bodygraph IHDS)."
    )
