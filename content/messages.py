...
TXT_CREATE_IMAGE_AFTER_MODEL = (
    ...
    "затем текстовый промпт — порядок не важен.\n\n"
    "После результата можно нажать <b>✏️ Доработать</b> или ответить на картинку текстом (15 мин)."
)
TXT_PHOTO_REFINE_PROMPT = (
    "✏️ <b>Режим доработки.</b> Опиши одним сообщением, что изменить на последнем изображении.\n\n"
    "Чтобы вставить второе фото (логотип, детский снимок, предмет) — отправь его "
    "<b>с подписью</b>, что с ним сделать (например: «надеть принт на футболку»)."
)
