...
async def test_send_chart_then_document_separately() -> None:
    ...
    with patch(
        ...
        ok = await send_table_generator_pack(message, raw_json)

    assert ok is True
    message.answer_photo.assert_awaited_once()
    photo_kwargs = message.answer_photo.await_args.kwargs
    assert len(photo_kwargs["caption"]) < _CAPTION_SAFE_MAX
    assert "Август" in photo_kwargs["caption"]
...
async def test_send_short_table_in_document_caption() -> None:
    ...
    assert short_table in doc_caption or "A" in doc_caption
